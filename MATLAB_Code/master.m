...
GSD_override = str2double(getenv('TFG_GSD'));
if ~isnan(GSD_override)
    GSD = GSD_override;
end

output_root = getenv('TFG_OUTPUT_ROOT');
if ~isempty(output_root)
    if ~exist(output_root, 'dir'), mkdir(output_root); end
    previous_dir = pwd;
    cd(output_root);
    cleanup_output_dir = onCleanup(@() cd(previous_dir));
end
...
params.MTF_Requirement   = MTF_Requirement;
params.SNR_Requirement   = SNR_Requirement;
params.GSD               = GSD;
