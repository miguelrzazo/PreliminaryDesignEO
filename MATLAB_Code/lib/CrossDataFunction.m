...
for config_idx = 1:size(configs, 1)
    ...
    Dmin_results = nan(length(alturas_orbitales), length(telescope_names), 3); % soluciones con cobertura y FOV
    Dmin_theoretical = nan(length(alturas_orbitales), length(telescope_names), 3); % solo MTF y SNR
        ...
        for telescopio_idx = 1:length(telescope_names)
            ...
            try
                ...
                for alt_idx = 1:length(alturas_orbitales)
                    ...
                    theoretical_diameter_indices = find(mtf_row >= MTF_Requirement & snr_row >= SNR_Requirement);
                    if ~isempty(theoretical_diameter_indices)
                        theoretical_diam_col_idx = theoretical_diameter_indices(1);
                        if theoretical_diam_col_idx <= length(diametros_pupila)
                            Dmin_theoretical(alt_idx, telescopio_idx, detector_idx) = diametros_pupila(theoretical_diam_col_idx);
                        end
                    end
    ...
    theoretical_fig = figure('Position', [100, 100, 900, 700], 'Visible', 'off');
    ...
    for detector_idx = 1:3
        ...
        legend_entries = {};
        for telescopio_idx = 1:length(telescope_names)
            dmin_values = Dmin_theoretical(:, telescopio_idx, detector_idx);
        ...
        title(sprintf('Detector %d: minimo teorico por MTF/SNR - %s', detector_idx, config_title_tex), ...
              'Interpreter', 'latex', 'FontSize', 10);
        xlabel('Altura orbital (km)', 'Interpreter', 'latex', 'FontSize', 9);
        ylabel('$D_{min}$ teorico (mm)', 'Interpreter', 'latex', 'FontSize', 9);
    ...
    theoretical_plot_base = sprintf('%s/HvsDmin_Teorico_%s', output_dir, config_names{config_idx});
    theoretical_fig.Color = 'white';
    set(findall(theoretical_fig, 'Type', 'Axes'), 'Color', 'white');
    saveas(theoretical_fig, [theoretical_plot_base '.jpg']);
    exportgraphics(theoretical_fig, [theoretical_plot_base '.pdf'], 'ContentType', 'vector');
    close(theoretical_fig);
    ...
    for detector_idx = 1:3
        ...
        title_str = sprintf('$h$ vs $D_{min}$ viable - Detector %d - %s', detector_idx, config_title_tex);
    ...
    main_plot_base = sprintf('%s/HvsDmin_Viable_%s', output_dir, config_names{config_idx});
    main_plot_filename = [main_plot_base '.jpg'];
    saveas(main_fig, main_plot_filename);
    saveas(main_fig, sprintf('%s/HvsDmin_%s.jpg', output_dir, config_names{config_idx}));
    main_fig.Color = 'white';
    set(findall(main_fig, 'Type', 'Axes'), 'Color', 'white');
    exportgraphics(main_fig, [main_plot_base '.pdf'], 'ContentType', 'vector');
    close(main_fig);
    ...
    if length(telescope_names) >= 1 
        ...
        if ~isempty(legend_labels_refractivo) % Solo configurar y guardar si hay datos
            ...
            refractivo_plot_base = sprintf('%s/HvsDmin_Viable_%s_Config_%s', output_dir, strrep(telescope_names{1},' ','_'), config_names{config_idx});
            refractivo_plot_filename = [refractivo_plot_base '.jpg'];
            saveas(refractivo_fig, refractivo_plot_filename);
            saveas(refractivo_fig, sprintf('%s/HvsDmin_%s_Config_%s.jpg', output_dir, strrep(telescope_names{1},' ','_'), config_names{config_idx}));
            refractivo_fig.Color = 'white';
            set(findall(refractivo_fig, 'Type', 'Axes'), 'Color', 'white');
            exportgraphics(refractivo_fig, [refractivo_plot_base '.pdf'], 'ContentType', 'vector');
        end
    ...
    for detector_idx = 1:3
        ...
        theoretical_output_data = [alturas_orbitales', Dmin_theoretical(:, :, detector_idx)];
        theoretical_output_table = array2table(theoretical_output_data, 'VariableNames', variable_names_table);
        theoretical_csv_filename = sprintf('HvsDmin/HvsDmin_Teorico_%s_Detector%d.csv', config_names{config_idx}, detector_idx);
        writetable(theoretical_output_table, theoretical_csv_filename);
