...
% === GRÁFICAS ADICIONALES: Solo telescopio 1 (Refractivo) cruzado con cada detector ===
figure('Position', [100, 100, 900, 700]);
for detector_idx = 1:3
    dmin_refractivo = Dmin_results(:, 1, detector_idx); % Telescopio 1 = columna 1
    valid_indices = ~isnan(dmin_refractivo);
    subplot(3,1,detector_idx);
    plot(alturas_orbitales(valid_indices), dmin_refractivo(valid_indices), 'b-o', 'LineWidth', 2, 'MarkerSize', 5);
    grid on;
    xlabel('Altura orbital (km)', 'Interpreter', 'latex', 'FontSize', 11);
    ylabel('Dmin Refractivo (mm)', 'Interpreter', 'latex', 'FontSize', 11);
    title(sprintf('Config: %d sat, %d tel | Telescopio: Refractivo | Detector %d', ...
        N_sat, N_telescopes, detector_idx), 'Interpreter', 'latex', 'FontSize', 12);
end
sgtitle(sprintf('H vs Dmin para Telescopio Refractivo - %s', config_names{config_idx}), 'Interpreter', 'latex', 'FontSize', 14);

% Guardar la figura
saveas(gcf, sprintf('%s/HvsDmin_Refractivo_%s.png', output_dir, config_names{config_idx}));
