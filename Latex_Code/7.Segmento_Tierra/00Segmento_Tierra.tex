...
La misión emplea dos canales de comunicaciones diferenciados. El canal de descarga de datos de observación opera en \textbf{banda X} (8\,GHz) y constituye el enlace principal de alta velocidad estudiado en este apartado. El canal de telemetría y telecomando (TM/TC), de baja tasa, funciona en banda UHF (437\,MHz) o S y es el soporte operativo de la misión: a través de él se envían comandos al satélite y se recibe información de estado durante cada pase. Este enlace de control se rige por el protocolo \textbf{CSP (\textit{CubeSat Space Protocol})} \cite{gomspace_csp_2023}, estándar \textit{de facto} en misiones de pequeños satélites: un protocolo de red ligero y direccionable —análogo funcional de IP/TCP— que permite la gestión de servicios a bordo (ping, transferencia de archivos, ejecución de comandos) con un \textit{overhead} mínimo, compatible con los recursos limitados de los microcontroladores embarcados en CubeSats.
...
\subsubsection{Balance de enlace (Link Budget)}

La capacidad máxima del enlace de descarga en banda X se fundamenta en la ecuación de Friis para la potencia recibida:
\begin{equation}
    P_r\,[\text{dBW}] = P_t + G_t - L_{\text{FSPL}} - L_{\text{atm}} + G_r - L_{\text{sys}}
\end{equation}
donde $P_t$ es la potencia de transmisión, $G_t$ y $G_r$ las ganancias de antena del satélite y la estación terrestre respectivamente, $L_{\text{FSPL}}$ la pérdida de espacio libre, $L_{\text{atm}}$ la atenuación atmosférica y $L_{\text{sys}}$ las pérdidas de sistema.

La \textbf{pérdida de espacio libre} (FSPL) para una frecuencia $f$ y una distancia $d$ es:
\begin{equation}
    L_{\text{FSPL}}\,[\text{dB}] = 20\log_{10}\!\left(\frac{4\pi d f}{c}\right)
\end{equation}
Para la geometría de paso a elevación media ($\approx 60°$), la distancia satélite-estación es $d \approx 700\,\text{km}$, lo que a 8,1\,GHz produce $L_{\text{FSPL}} = 167\,\text{dB}$.

La figura de mérito de la estación receptora se define como la relación antena-temperatura de ruido, $G/T$. Para la antena de 13\,m de Fairbanks operando en banda X, la ganancia se estima mediante:
\begin{equation}
    G_r\,[\text{dBi}] = 10\log_{10}\!\left[\eta\left(\frac{\pi D}{\lambda}\right)^2\right]
\end{equation}
con eficiencia $\eta = 0{,}55$ y diámetro $D = 13\,\text{m}$, lo que da $G_r \approx 58\,\text{dBi}$. Con una temperatura de sistema $T_{\text{sys}} \approx 50\,\text{K}$, se obtiene $G/T \approx 41\,\text{dB/K}$.

A partir de estos valores se construye el \textbf{balance de enlace} (Tabla~\ref{tab:link_budget}), que evalúa la relación $E_b/N_0$ obtenida frente a la requerida por la modulación empleada (QPSK, BER $= 10^{-6}$):

\begin{table}[H]
\centering
\caption{Balance de enlace del canal de descarga en banda X (escenario nominal, elevación $60°$).\\ Fuente: Elaboración propia.}
\label{tab:link_budget}
\begin{tabular}{lcc}
\hline
\textbf{Parámetro} & \textbf{Símbolo} & \textbf{Valor} \\
\hline
Potencia de transmisión                       & $P_t$                 & 5 dBW           \\
Ganancia antena satélite (banda X, parche)    & $G_t$                 & 8 dBi           \\
\textbf{EIRP}                                 &                       & \textbf{13 dBW} \\
\hline
Pérdida de espacio libre (700 km, 8,1 GHz)   & $L_\text{FSPL}$       & $-167$ dB       \\
Atenuación atmosférica (nominal)              & $L_\text{atm}$        & $-0{,}5$ dB     \\
\hline
Ganancia antena terrestre (Fairbanks, 13 m)   & $G_r$                 & 58 dBi          \\
Temperatura de ruido del sistema              & $T_\text{sys}$        & 50 K            \\
Figura de mérito                              & $G/T$                 & 41 dB/K         \\
\hline
Constante de Boltzmann                        & $k$                   & $-228{,}6$ dBW/Hz \\
\textbf{Densidad portadora-ruido}             & $C/N_0$               & \textbf{115,1 dB·Hz} \\
\hline
Velocidad de datos                            & $R_b$                 & 150 Mbps (81,8 dB·Hz) \\
$E_b/N_0$ obtenida                           &                       & 33,3 dB         \\
$E_b/N_0$ requerida (QPSK, BER $10^{-6}$)   &                       & 10,5 dB         \\
\hline
\textbf{Margen de enlace}                     &                       & \textbf{22,8 dB} \\
\hline
\end{tabular}
\end{table}

El margen de 22,8\,dB obtenido en condiciones nominales confirma que la tasa de 150\,Mbps indicada por el fabricante es alcanzable y que el enlace dispone de un margen holgado para absorber las pérdidas operativas analizadas en los apartados siguientes.
...
\begin{itemize}
    ...
    \item \textbf{Efecto Doppler}: El movimiento orbital del satélite a $v \approx 7{,}6\,\text{km/s}$ produce un desplazamiento en frecuencia de la portadora según:
    \begin{equation}
        \Delta f = f_0 \cdot \frac{v_r}{c}
    \end{equation}
    donde $v_r$ es la componente radial de la velocidad relativa satélite-estación. Para la banda X ($f_0 = 8{,}1\,\text{GHz}$) el desplazamiento máximo, producido cuando el satélite asciende o desciende por el horizonte, alcanza $|\Delta f|_\text{máx} \approx \pm\,205\,\text{kHz}$. La tasa de variación máxima, en el instante de máxima elevación (\textit{TCA}), es aproximadamente $3{,}1\,\text{kHz/s}$. La Figura~\ref{fig:doppler} muestra el perfil de desplazamiento Doppler a lo largo de un pase representativo. El receptor de la estación incorpora un \textit{phase-locked loop} (PLL) de seguimiento que compensa este barrido de frecuencia en tiempo real; el efecto queda recogido dentro del margen de enlace calculado previamente y no supone una penalización adicional sobre la velocidad de descarga.

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.85\linewidth]{7.Segmento_Tierra/doppler_profile.pdf}
        \caption{Perfil de desplazamiento Doppler en banda X durante un pase representativo. \\ Fuente: Elaboración propia (MATLAB, \texttt{calcularDoppler.m})}
        \label{fig:doppler}
    \end{figure}

    \item \textbf{Polarización}: La antena transmisora del satélite (típicamente una \textit{turnstile} de polarización circular) y la antena receptora de Fairbanks (plato parabólico con alimentación de polarización circular) están alineadas en polarización, por lo que la pérdida por desadaptación es $\approx 0\,\text{dB}$. En misiones futuras que utilicen antenas de polarización lineal en tierra, esta pérdida puede alcanzar los 3\,dB y deberá incluirse como partida explícita en el balance de enlace.
\end{itemize}
...
Los efectos Doppler y de polarización quedan englobados dentro del margen de enlace calculado y del factor corrector de hardware, dado que el receptor incorpora compensación activa de frecuencia y la estación de Fairbanks opera con antenas de polarización circular. Existen otros fenómenos que pueden hacer fluctuar la velocidad real de descarga, pero en este trabajo se han considerado los de mayor peso.
...
\begin{equation}
t_{\text{descarga}} = \frac{8{,}843\ \text{GB}}{76{,}1\ \text{Mbps} \times \frac{1}{8}\ \text{MB/s}} = \frac{8843}{9{,}5} = \boxed{931\ \text{segundos}}
\end{equation}
