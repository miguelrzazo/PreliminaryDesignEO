...
Una órbita de traza repetida (RGT) es aquella en la que el satélite
repite su proyección sobre la superficie terrestre tras un número entero
de días $D$ y un número entero de órbitas $N$. Estas órbitas permiten
adquirir imágenes de una misma zona bajo condiciones geométricas similares
a intervalos regulares.

En una órbita heliosíncrona el plano orbital precesa solidario con el Sol.
La condición RGT se formula mediante las tasas orbitales y la rotación
sideral de la Tierra:

\begin{equation}
 n_k + \dot M_{J_2} + \dot\omega_{J_2}
= \left(\omega_\oplus - \dot\Omega_{J_2}\right)\frac{N}{D}
\label{eq:rgt_cond}
\end{equation}

donde $n_k=\sqrt{\mu/a^3}$, $\omega_\oplus=2\pi/T_{\mathrm{rot}}$ y
$T_{\mathrm{rot}}=86164.0905\ \si{s}$. El semilatus rectum es
$p=a(1-e^2)$ y las tasas se calculan como:

\begin{align}
\dot\Omega_{J_2} &= -\frac{3}{2} n_k J_2
\left(\frac{R_T}{p}\right)^{\!2}\cos i
\label{eq:Omega_dot}\\[4pt]
\dot\omega_{J_2} &= \frac{3}{4} n_k J_2
\left(\frac{R_T}{p}\right)^{\!2}\left(4-5\sin^2 i\right)
\label{eq:omega_dot}
\\[4pt]
\dot M_{J_2} &= \frac{3}{4} n_k J_2
\frac{\sqrt{1-e^2}\,R_T^2}{a^2(1-e^2)^2}
\left(2-3\sin^2 i\right)
\label{eq:M_dot_J2}
\end{align}

Para presentar el resultado orbital se define el período orbital efectivo:

\begin{equation}
T_{\mathrm{orb,eff}} = \frac{2\pi}
{n_k+\dot M_{J_2}+\dot\omega_{J_2}}
\label{eq:Torb_eff}
\end{equation}

La condición heliosíncrona \eqref{eq:SSO_cond} fija $i=i_{\mathrm{SSO}}(h)$.
Al sustituirla, la ecuación \eqref{eq:rgt_cond} queda en función de la altura
y se puede resolver para cada pareja de enteros $(N,D)$
\cite{vallado2013fundamentals}.

\subsubsection{Órbitas \textit{frozen}}

Una órbita \textit{frozen} se diseña para reducir las derivas seculares de la
excentricidad y del argumento del perigeo. Para el régimen de pequeña
excentricidad, $e \ll 1$, la condición de primer orden se obtiene del
...
rompe la simetría entre los hemisferios norte y sur del campo gravitatorio.

La condición \textit{frozen} no elimina las variaciones de $e$ y $\omega$.
Elimina las derivas seculares debidas al acoplamiento entre $J_2$ y $J_3$,
mientras que las componentes de largo periodo asociadas a $J_3$ persisten.
Ambos parámetros continúan oscilando, con una amplitud menor que en una
órbita no \textit{frozen}. La selección de $e$ y $\omega$ busca así limitar la
variación de la altura orbital y conservar una geometría de observación
estable a lo largo de la misión.
