\chapter{Conceptos Previos}\label{ch:conceptos}
\minitoc

Este apartado sienta las bases teóricas necesarias para definir la carga útil y buscar la solución de diseño.
...
\subsection{Frecuencia de Nyquist\texorpdfstring{($f_{Ny}$)}{(fNy)}}
...
donde el \text{tamaño del píxel} actúa como el intervalo de muestreo espacial.
...
\begin{itemize}
  ...
  Para un telescopio con obstrucción central\footnote{obstrucción central que bloquea parte de la luz (normalmente un espejo) que entra al sistema óptico, siendo $d_{obs}$ la medida de dicha obstrucción}, como es el caso de los telescopios Cassegrain y Korsch, su MTF de difracción vendrá definida, en función del diámetro de obstrucción $d_{obs}$. Para el presente trabajo se establecerá $R_{obs} = 0,2$ como valor inicial.\cite{Foadi2023DesigningCT}
\begin{equation}
\mathrm{OTF}_{\mathrm{diff}} = \frac{A + B + C}{1 - R^2}
\end{equation}

\begin{equation}\label{eq:mtfrobs}
...
\subsection{\textit{Signal To Noise Ratio} (SNR)}\label{sec:snr}
\begin{figure}[H]
    \centering
...
La \textit{Signal-to-Noise Ratio} (SNR, o relación señal-ruido) es un indicador clave que indica la calidad radiométrica de un sistema de observación óptica. Representa la relación entre la señal útil (fotones detectados provenientes de la escena) y el ruido total (suma de todas las fuentes de ruido del sistema) en el detector. Un SNR alto significa que la señal es claramente distinguible del ruido, lo que permite obtener imágenes precisas y fiables, mientras queun SNR bajo implica que la señal está enmascarada por el ruido, dificultando la detección de detalles y la calidad de los datos obtenidos\cite{design_workshop_optical_2023}\cite{zorita_snr_2023}.
...
La irradiancia \( E \) en el plano focal se estima mediante:

\begin{equation}
E = \frac{\pi \cdot \tau \cdot \Delta \lambda \cdot L_{\mathrm{ref}}}{1 + 4\,T\#^{2}}
\end{equation}

donde \( F\# = \frac{f}{D} \) es la relación focal del sistema, con \( f \) la distancia focal y \( D \) el diámetro de la apertura.\footnote{Para los sistemas con obstrucción central se utilizará la relación focal equivalente $T\#$, definida más adelante en la ecuación~\eqref{eq:relfoc}}.
...
\begin{itemize}
    ...
    \item \( TDI \) es el número de etapas del sistema Time Delay Integration.\footnote{Dicho concepto se explicará más adelante en \ref{sec:tdi}}.
...
donde \( \mu = 3{,}986 \times 10^{14} \, \si{m^3/s^2} \) es el parámetro gravitacional estándar de la Tierra y \( R_T = 6371 \, \si{km} \) es el radio medio terrestre. Este tiempo de integración se utiliza directamente en el cálculo de \( N_e \), y representa el tiempo efectivo durante el cual se acumulan fotones en cada píxel del detector. Se emplea la velocidad orbital, ligeramente superior a la velocidad de la traza sobre el suelo, lo que proporciona un tiempo de integración conservador.
...
En la implementación numérica se sustituye \(\sigma_{\text{stray}}\) por \(N_{\text{nonlin}}\).
...
Siendo $N_{pix}$ el número de píxeles situados en línea perpendicular al sentido de movimiento del satélite.
...
Siendo $h$ la altura orbital.

En este estudio se supone que solo se observará al \textbf{nadir}, es decir, el instrumento siempre apunta directamente hacia abajo, al punto situado justo debajo del satélite en la superficie terrestre (\textit{sub satellite point}). Esto significa que no se realizarán observaciones oblicuas (\textit{off-nadir}), lo que simplifica el diseño óptico y la planificación de misión. Asimismo, el punto subsatélite es el punto de máxima resolución y mínima distorsión geométrica en la imagen captada, con lo que se maximiza la calidad geométrica y radiométrica de la imagen y se simplifica la calibración y el procesamiento de datos. Además, el cálculo del GSD y del MTF es directo, ya que no hay degradación por ángulo de observación. Tampoco se considerará la deformación producida por la esfericidad de la Tierra, considerando la superficie a capturar como plana, ni las deformaciones producidas en los extremos del array debido a su proyección.
...
Para operar en el espacio, estos detectores deben superar condiciones particularmente exigentes: radiación cósmica, variaciones extremas de temperatura y ausencia de mantenimiento físico. Además, se requiere alta sensibilidad en las longitudes de onda de interés, bajo nivel de ruido para detectar señales débiles y un consumo energético reducido, dado que las plataformas de satélite disponen de recursos limitados. Parámetros como la resolución espacial, temporal y radiométrica son determinantes en la calidad de los datos adquiridos y su utilidad en distintas aplicaciones científicas y operativas \cite{kuroda_essential_2014}.
...
Los sensores CCD (Dispositivo de Carga Acoplada) han sido durante años el estándar en misiones de observación espacial, gracias a su excelente calidad de imagen y rendimiento en condiciones de baja iluminación. Funcionan acumulando carga en pozos de potencial dentro de un sustrato semiconductor, transfiriendo luego estas cargas secuencialmente a un nodo de salida para su lectura.

Destacan por su amplio rango dinámico, que puede duplicar el de los CMOS, y su bajo ruido de lectura, ideal para observaciones nocturnas o en bandas espectrales de baja radiancia. Presentan una notable uniformidad entre píxeles, reduciendo el ruido de patrón fijo y facilitando la calibración radiométrica. Además, al tener menos electrónica integrada en cada píxel, ofrecen un mayor factor de relleno, aprovechando mejor el área fotosensible.
...
\begin{figure}[H]
    ...
    \label{fig:whiskbroom-operation}
\end{figure}
...
Para dimensionar la misión se establece un límite máximo de \textbf{tres detectores}, con el fin de simplificar la disposición de los mismos \cite{design_workshop_optical_2023}.
...
La relación focal se define como el cociente entre la distancia focal y el diámetro de pupila ($F\#=\frac{f}{D}$). Para los sistemas con obstrucción central, se define la relación focal equivalente $T\#$, que viene dada por la relación:
\begin{align}
\label{eq:relfoc}
    T\# = \frac{f}{\sqrt{D^2(1-R_{obs}^2)}} 
\end{align}

Siendo $R_{obs}$ la misma relación de obstrucción en la ecuación~\eqref{eq:mtfrobs}.
Es un parámetro clave ya que un número $F\#$ bajo (óptica «rápida») implica una apertura grande respecto a la distancia focal, lo que permite captar más luz y mejora la SNR, pero puede aumentar las aberraciones ópticas y reducir la profundidad de campo. Un número $F\#$ alto (óptica «lenta») reduce la cantidad de luz captada, pero suele simplificar el diseño y controlar mejor las aberraciones. La relación focal afecta directamente a la MTF, la SNR y la saturación del detector.
...
El telescopio Korsch es un reflector de tres espejos, también con obstrucción central. Su diseño está optimizado para controlar la luz parásita (straylight). Permite focales largas y una buena calidad de imagen en todo el campo, aunque el alineamiento es más complejo y la penalización en la MTF por este motivo es mayor ($MTF_{alineamiento} \approx 0,80$). La presencia de la obstrucción central sigue penalizando la MTF de difracción, aunque el control de aberraciones es excelente. Posee un FoV similar al Cassegrain, de $3^\circ$.
...
El telescopio de tres espejos anastigmático es un reflector de tres espejos sin obstrucción central, ya que el haz óptico se desvía fuera del eje óptico principal (diseño off-axis). Esto permite un campo de visión más amplio, del orden de $8^\circ$, y una calidad de imagen homogénea en todo el campo, sin penalización por la presencia de elementos que bloqueen la apertura. Sin embargo, el alineamiento de un TMA es extremadamente exigente, con una penalización en la MTF de alineamiento significativa ($MTF_{alineamiento} \approx 0,70$). Además, la fabricación y verificación de estos sistemas es más compleja y costosa.
...
\hline
\textbf{Tipo} & \makecell{\textbf{Obstrucción}\\\textbf{$R_{obs}$}} & \textbf{$FoV_{lim}$} & \makecell{\textbf{MTF}\\\textbf{alineamiento}} & \makecell{\textbf{Transmisión óptica}\\\textbf{$(\tau)$}} \\
\hline
Refractivo\textsuperscript{*}    & No  & 10º & 0,90 & 0,8 \\
...
\par\smallskip
\noindent\textsuperscript{*}\footnotesize Limitado a 80 mm de diámetro máximo válido para el presente trabajo.
...
A continuación se definen los principales parámetros que definen la órbita del cuerpo de interés, así como las perturbaciones asociadas de mayor relevancia en la mecánica orbital para el presente problema \cite{curtis2020orbital}.
...
\noindent En el análisis preliminar, el tamaño de una órbita terrestre puede
expresarse convenientemente mediante la altura orbital, definida como
$h=a-R_T$, siendo $R_T$ el radio de la Tierra. La excentricidad se conserva
como parámetro independiente para distinguir entre una aproximación circular
y una órbita ligeramente elíptica.

\subsection{Inclinación \texorpdfstring{($i$)}{(i)}}
...
\subsection{Argumento del Perigeo \texorpdfstring{($\omega$)}{(omega)}}
...
Debido a que en este estudio solo se aplicarán órbitas circulares ($e\approx0$), este parámetro carece de relevancia, pues no habrá un perigeo claramente definido.

\subsection{Anomalía Verdadera \texorpdfstring{($\nu$)}{(nu)}}
...
Estas perturbaciones dependen de la altitud, la inclinación y otros parámetros orbitales. En el próximo apartado (\ref{sec:SSO}) se detalla cómo acoplar dichas variables para definir órbitas heliosíncronas.
...
El problema del tercer cuerpo se refiere a la dinámica gravitacional que surge cuando tres objetos celestes (en nuestro caso, el sistema Tierra-Luna- satélite, o Tierra-Sol- satélite) interactúan entre sí mediante la fuerza de gravedad. A diferencia del problema de dos cuerpos, que tiene una solución matemática exacta y predecible, la introducción de un tercer cuerpo introduce un nivel de complejidad significativo.
...
El tratamiento analítico de este problema es extraordinariamente difícil porque la perturbación depende explícitamente del tiempo. Las ecuaciones diferenciales que describen este sistema no tienen una solución cerrada o analítica exacta, lo que ha llevado al desarrollo de métodos numéricos para aproximar las soluciones.

El problema de los tres cuerpos es intrínsecamente caótico, lo que significa que es extremadamente sensible a las condiciones iniciales. Pequeñas variaciones en la posición o velocidad inicial de cualquiera de los cuerpos pueden resultar en trayectorias completamente diferentes a largo plazo. Esto imposibilita predicciones precisas sin conocer con exactitud absoluta las condiciones iniciales.\\

\subsection{Órbitas de especial interés}

\subsubsection{Órbitas heliosíncronas}\label{sec:SSO}

Las órbitas heliosíncronas ajustan la precesión del nodo ascendente para conservar una hora solar local casi constante. Esta propiedad interesa en observación óptica pasiva porque el instrumento mide radiación solar reflejada; mantener el LTAN reduce cambios de iluminación entre pasadas.

El achatamiento terrestre, modelado mediante \(J_2\), permite imponer esa precesión. Para una órbita casi circular, se escoge la inclinación de modo que la regresión nodal iguale la velocidad angular aparente del Sol respecto a la Tierra (\( \approx 0{,}9856^\circ/\text{día} \)):
...
\begin{itemize}
    ...
    \item \( r \) es el radio orbital (radio de la Tierra más la altitud del satélite),
    ...
    \item \( R_T=R_{\oplus}\approx 6378{,}137 \ \text{km} \) es el radio ecuatorial de la Tierra,
    \item \( \mu \approx 3{,}986004418 \times 10^{14} \ \text{m}^3/\text{s}^2 \) es el parámetro gravitacional estándar de la Tierra.
\end{itemize}

\subsubsection{Órbitas de Traza Repetida}

Una órbita de traza repetida (RGT) repite la proyección del satélite sobre
la superficie terrestre tras $k_{\mathrm{day2rep}}$ rotaciones terrestres y
$k_{\mathrm{orb2rep}}$ órbitas. La condición RGT exige que la frecuencia
angular nodal media coincida con la velocidad angular de la traza respecto
a la Tierra:

\begin{equation}
n_k(a) + \dot{M}_1(a,i,e) + \dot{\omega}(a,i,e)
= \left[\omega_{\oplus} - \dot{\Omega}(a,i,e)\right]
\frac{k_{\mathrm{orb2rep}}}{k_{\mathrm{day2rep}}}
\label{eq:rgt_cond}
\end{equation}

donde $n_k(a)=\sqrt{\mu/a^3}$ es el movimiento medio de la órbita
kepleriana y $\omega_{\oplus}$ es la velocidad angular de rotación terrestre respecto al sistema inercial. En esta condición se emplea el periodo sideral:

\begin{equation}
T_{\oplus}=\frac{2\pi}{\omega_{\oplus}}=86164{,}09\ \mathrm{s},
\qquad
\omega_{\oplus}=7{,}292115373\times10^{-5}\ \mathrm{rad/s}.
\label{eq:earth_sidereal}
\end{equation}

Las cantidades $\dot{\Omega}$, $\dot{\omega}$ y $\dot{M}_1$ son las tasas seculares asociadas a $J_2$. Las expresiones generales de estas tasas son \cite{vallado2013fundamentals}:

\begin{align}
\dot{\Omega}_{J_2} &= -\frac{3J_2 n R_{\oplus}^{2}}{2p^{2}}\cos i,
\label{eq:Omega_dot}\\[4pt]
\dot{\omega}_{J_2} &= \frac{3J_2 n R_{\oplus}^{2}}{4p^{2}}
\left(4-5\sin^{2}i\right),
\label{eq:omega_dot}\\[4pt]
\dot{M}_{1,J_2} &= \frac{3J_2 n\sqrt{1-e^{2}}R_{\oplus}^{2}}{4p^{2}}
\left(2-3\sin^{2}i\right),
\label{eq:M1_dot}
\end{align}

con $n=n_k(a)$, $p=a(1-e^2)$ y $a=R_{\oplus}+h$. Para el caso
circular, $e=0$ y $p=a$; por tanto, el período nodal se obtiene como caso
particular de la expresión general:

\begin{equation}
T_{\mathrm{nodal}} = \frac{2\pi}
{n_k(a)+\dot{M}_{1,J_2}(a,i,0)+\dot{\omega}_{J_2}(a,i,0)}
=\frac{T_k}
{1+\frac{3}{4}J_2\left(\frac{R_{\oplus}}{a}\right)^2
\left(6-8\sin^2 i\right)},
\qquad T_k=2\pi\sqrt{\frac{a^3}{\mu}}
\label{eq:Tnodal}
\end{equation}

La condición heliosíncrona \eqref{eq:SSO_cond} determina
$i=i_{\mathrm{SSO}}(h)$. Al sustituir esta inclinación en
\eqref{eq:rgt_cond}, el modelo resuelve una ecuación para cada pareja de
enteros $(k_{\mathrm{orb2rep}},k_{\mathrm{day2rep}})$
\cite{vallado2013fundamentals}.

\subsubsection{Órbitas \textit{frozen}}

Una órbita \textit{frozen} se obtiene al escoger la excentricidad y la
orientación de la órbita para cancelar, en la aproximación de primer orden,
los términos seculares dominantes asociados a $J_3$ en la evolución de $e$ y
$\omega$. Esta condición reduce las variaciones seculares de ambos
parámetros, pero no las elimina por completo. Para $e\ll1$ y la orientación
analizada en este trabajo, $\omega=90^\circ$, la excentricidad se aproxima
mediante \cite{vallado2013fundamentals}:

\begin{equation}
e_{\textit{frozen}} \approx -\frac{J_3}{2J_2}\,\frac{R_T}{a}\,\sin i,
\qquad e\ll1,\quad \omega=90^\circ
\label{eq:frozen_cond}
\end{equation}

donde $J_3 \approx -2{,}5327 \times 10^{-6}$ es el armónico zonal impar que
rompe la simetría norte-sur del campo gravitatorio. Aunque $\omega=270^\circ$
puede aparecer como orientación alternativa en formulaciones generales, esta
orientación no se considera en el presente análisis.

El interés de esta condición para una misión de observación de la Tierra
reside en que limita la deriva secular de la geometría orbital. Por tanto,
el perfil de altura y la geometría de observación se repiten con menor
variación entre órbitas, aunque permanecen variaciones residuales debidas a
los términos no cancelados y a las perturbaciones que no se incluyen en esta
aproximación.

La oscilación de altura asociada es $\Delta h = 2 a e_{\textit{frozen}}$.
Para que la condición \textit{frozen} sea relevante, $\Delta h$ debe ser menor que
la banda muerta de mantenimiento orbital y el decaimiento atmosférico no
debe dominar sobre las derivas seculares. A altitudes bajas la densidad
crece y el decaimiento continuo del semieje mayor enmascara el beneficio
de la condición \textit{frozen} \cite{vallado2013fundamentals}.


\subsection{Consideración de las perturbaciones orbitales}


Para el presente trabajo no se modelan la presión de radiación solar ni el problema del tercer cuerpo. En una órbita circular de 630 km, la aceleración gravitatoria terrestre vale aproximadamente \(8{,}1\ \mathrm{m/s^2}\). La perturbación lunar queda del orden de \(10^{-6}\ \mathrm{m/s^2}\), y la solar en torno a \(10^{-7}\ \mathrm{m/s^2}\). La presión de radiación solar, para un satélite compacto con relación área/masa cercana a \(0{,}04\ \mathrm{m^2/kg}\), queda alrededor de \(10^{-7}\ \mathrm{m/s^2}\). Estas contribuciones son pequeñas frente al término central y no gobiernan la selección preliminar de altura \cite{vallado2013fundamentals}.

Sí se incluyen dos efectos porque condicionan directamente la misión: \(J_2\), que fija la inclinación heliosíncrona y el LTAN, y la resistencia aerodinámica, que determina el decaimiento orbital y la masa de combustible.
