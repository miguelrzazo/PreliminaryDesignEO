...
La traza se repite respecto al día solar, no al día sidéreo. La condición
RGT para una órbita SSO es:

\begin{equation}
N \cdot T_{\mathrm{nodal}} = D \cdot T_{\mathrm{solar}},
\qquad T_{\mathrm{solar}} = 86400\ \si{s}
\label{eq:rgt_cond}
\end{equation}

donde $T_{\mathrm{nodal}}$ es el período entre dos cruces consecutivos del
nodo ascendente. El achatamiento terrestre $J_2$ modifica este período
respecto del kepleriano mediante dos efectos seculares. Para $e=0$:

\begin{align}
\dot{\Omega} &= -\frac{3}{2}\, n\, J_2 \left(\frac{R_T}{a}\right)^{\!2} \cos i
\label{eq:Omega_dot}\\[4pt]
\dot{\omega} &= \frac{3}{4}\, n\, J_2 \left(\frac{R_T}{a}\right)^{\!2}
\bigl(5\cos^2 i - 1\bigr)
\label{eq:omega_dot}
\end{align}

con $n = \sqrt{\mu / a^3}$ y $a = R_T + h$. El período nodal que combina
ambos efectos es:

\begin{equation}
T_{\mathrm{nodal}} = \frac{T_k}
{1 + \frac{3}{4}J_2\left(\frac{R_T}{a}\right)^{\!2}\!\bigl(6 - 8\sin^2 i\bigr)},
\qquad T_k = 2\pi\sqrt{\frac{a^3}{\mu}}
\label{eq:Tnodal}
\end{equation}

La condición heliosíncrona \eqref{eq:SSO_cond} fija $i = i_{\mathrm{SSO}}(h)$.
Al sustituirla, $T_{\mathrm{nodal}}$ queda en función exclusiva de $h$ y la
ecuación \eqref{eq:rgt_cond} se puede resolver para cada pareja de enteros
$(N,D)$ \cite{vallado2013fundamentals}.
