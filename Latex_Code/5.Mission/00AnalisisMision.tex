...
Apreciando en esta gráfica el mínimo de masa a poner en orbita por satélite de \textbf{1,710 kg} (distribuidos como: $m_{\text{seca}}= 1{,}349\ \text{kg}; \quad m_{\text{comb}}= 0{,}361\ \text{kg}$) a una altura orbital de \textbf{520 km}, correspondiente a un diámetro de pupila de \textbf{28 mm}. Dado que la constelación estará formada por dos satélites, la masa total a inyectar en orbita será de \textbf{3,42 kg}.
...
\begin{table}[h]
  ...
  \begin{tabular}{@{}l S[table-format=1.3] S[table-format=2.1]@{}}
    ...
    \midrule
    \quad Paneles solares                    & 0,229    & 13,4         \\
    \quad Baterías                           & 0,013    & 0,8          \\
    \midrule
    Comunicaciones (banda X)                 & 0,270    & 15,8          \\
    Estructura                               & 0,251    & 14,7          \\
    Control de actitud                       & 0,050    & 2,9           \\
    Propulsión                               & 0,057    & 3,3          \\
    \midrule
...
La masa estructural representa típicamente entre 15--20\,\% de la masa total del satélite \cite{tsinas2001efficient}. Este porcentaje se mantiene relativamente constante debido a la escalabilidad de las cargas estructurales. Por tanto, se puede asumir que la masa estructural del satélite será de 0,251\,kg (14,7\,\%).

El diseño estructural se plantea conforme al estándar CubeSat \cite{calpoly2022cubesat}, con interfaz compatible con desplegadores PPOD, lo que garantiza compatibilidad con la mayoría de proveedores de lanzamiento en \textit{rideshare}. La envolvente de cargas de lanzamiento típica para cargas secundarias en este estándar es de 12\,g en dirección axial y 8\,g lateral (cuasi-estáticos), con niveles de vibración aleatoria de hasta 14,1\,g\,rms en el rango 20--2000\,Hz \cite{calpoly2022cubesat}. El bajo coeficiente de dilatación térmica del aluminio seleccionado ($\alpha \approx 23\ \mu\text{m}/(\text{m·K})$) minimiza las deformaciones termo-elásticas en órbita, lo que contribuye a mantener la alineación del instrumento óptico.
...
\]
Para una misión de monitorización de CO$_2$/O$_2$ en la que la geolocalización se refina en postproceso, se adopta un requisito de apuntamiento de $\leq 1°$, equivalente a un error de geolocalización de $\approx 9$\,km en tierra. Este margen es coherente con la resolución de los instrumentos de referencia (OCO-2, TROPOMI) y con las necesidades de detección de tendencias climáticas a escala regional.
...
La masa total del subsistema ADCS (barra de gradiente + magnetopares + margen) es de 0,05\,kg, representando el 2,9\,\% de la masa total del satélite.
...
Por tanto, el sistema de comunicaciones suma 270 g, o un 15,8\,\% de la masa total del satélite.
...
\begin{itemize}
    ...
    \item Transmisión de datos: Es el modo de mayor demanda energética, activado para la descarga de datos a la estación de Tierra. Los consumos son: transmisor EnduroSat X-Band (\SI{22}{W}) \cite{endurosat_xband_2025}, instrumento (\SI{0,2}{W}), ADCS (\SI{0,25}{W}) y OBC (\SI{0,4}{W}). El consumo total en este modo asciende a \textbf{\SI{22,85}{W}}.
\end{itemize}
...
\[
P_{\text{déficit}} = \SI{22,85}{W} - \SI{12,47}{W} = \SI{10,38}{W}.
\]
...
\[
E_{\text{bat}} = \SI{10,38}{W} \times 0{,}25\ \text{h} = \SI{2,60}{Wh}.
\]
Con una densidad energética de \(\SI{200}{Wh/kg}\) \cite{sunket2025lithium}, la masa mínima para el almacenamiento es de $2{,}60/200 = 0{,}013$\,kg. Se adopta \textbf{\SI{0,013}{kg}} como valor de diseño.

La masa total del subsistema eléctrico resultante (\(m_{\text{panel}} + m_{\text{batería}}\)) asciende a \SI{0,242}{kg}. Este valor es consistente con la asignación del presupuesto de masas, validando la configuración propuesta. Considerando que los requerimientos de potencia son laxos, pues el tiempo de transmisión de datos en continuo nunca superará más de los 15 minutos, y que para una órbita SSO amanecer-anochecer (LTAN\,=\,06:00\,h) el ángulo $\beta$ es elevado durante la mayor parte del año, los eclipses son breves o inexistentes en la mayoría de las fases de la misión, el dimensionado supone una aproximación favorable a lo esperado en la práctica.
